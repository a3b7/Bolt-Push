...
export default function AccountTab({ isAuthed, onAuthClick }: { isAuthed: boolean; onAuthClick: () => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const { data: prof } = await supabase
        .from('profiles').select('daily_goal, display_name').eq('user_id', user.id).maybeSingle()
  ...
  async function saveAll() {
    ...
    const { error } = await supabase.from('profiles').upsert({ user_id: user.id, daily_goal: n, display_name: trimmed })
    setSaving(false)
  ...
  async function logOut() {
    ...
    // After sign-out, App will render non-auth state; force a hard reload to clear any in-memory state.
